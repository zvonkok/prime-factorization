...
function ellmult21(x,s,a,N: integer): array[2];
var
    x1,xold,z,zinv,P1,P1inv,Pprime,mu,k: integer;
begin
    if s = 0 then return (0,0); end;
    x1 := x; z := 1;
    P1 := (x*x*x + a*x*x + x) mod N;
    writeln("P1  ", P1);
    P1inv := mod_inverse(P1,N);
    writeln("P1inv  ", P1inv);
    if P1inv = 0 then return (gcd(N,P1),0); end;
...
(*--------------------------------------------------------*)
function ec_fact0_meins(N,a,bound: integer): integer;
const
    anz0 = 128;
var
    x, B0, B1, s, d: integer;
...
begin
	...
	xx := ellmult21(x,s,a,N);
	writeln("ellmult21 :   ", xx);
	
	xx := mod_pemult(x,s,a,N);
	writeln("mod_pemult :   ", xx);
